...
We wish to investigate how well the Hopkins score could be predicted based on the stress measure. The data are provided in [Lab 08 - Stress.sav](Lab 08 - Stress.sav).
...
Ⓡ perform a Q-Q plot of the residuals with the `qqPlot()` function from the `car` package:
...
Ⓥ under the `Assumption Checks` section you'll find `Q-Q plot of residuals`
...
Ⓡ the F test statistic is available in the summary of the model:
      
```{r eval=FALSE}
summary(model)
```
      
Ⓥ the F test statistics is available in the `Omnibus ANOVA test`, available from the `Model Coefficients` section of the `Linear Regression` analysis.
...
Ⓡ confidence intervals for the parameters of the model can by obtained with the `confint()` function
      
```{r eval=FALSE}
confint(model)
```
      
Ⓥ confidence intervals for the parameters estimates are available under the `Model Coefficients` section.
...
Ⓡ create a new data frame with a `Stress` value of 35, and then pass our model and this new data frame to the `predict()` function. If we provide the argument `interval='confidence'` we'll receive a confidence interval too:
...
Ⓥ There's no easy way to do this in jamovi, so you can copy and paste the above code into the `Rj Editor`.
...
Ⓡ Same as the call to `predict()` in the previous question but with `interval='prediction'`.
...
Ⓥ Copy and paste the above code into the `Rj Editor`.
...
Hopkins symptoms score for the population of students who score Xh = 35 on the stress measure. The second interval is an estimate of where the Hopkins score for any one of those individuals might be. Notice the increased width of the latter.
